...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Bath gets higher priority)
    const isBath = location.slug === 'bath';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isBath ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isBath ? 1 : 2 + i),
    });
