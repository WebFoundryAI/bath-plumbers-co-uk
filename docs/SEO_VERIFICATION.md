...
**Canonical Host:** https://bathplumbers.co.uk (non-www, HTTPS)
...
curl -I https://bathplumbers.co.uk/robots.txt
curl -I https://bathplumbers.co.uk/sitemap.xml
curl -I https://bathplumbers.co.uk/llm.html
```
...
| Canonical format | `https://bathplumbers.co.uk/path` (no trailing slash except homepage) |
| Homepage canonical | `https://bathplumbers.co.uk/` |
...
- [x] `/locations/bath` - Location detail
- [x] `/locations/bath/blocked-drains` - Location + service
...
| Service pages | `https://bathplumbers.co.uk/services/blocked-drains` |
| Location pages | `https://bathplumbers.co.uk/locations/bath` |
...
curl https://bathplumbers.co.uk/sitemap.xml | grep -c '<url>'
```

**Expected sitemap content:**
- All `<loc>` URLs use `https://bathplumbers.co.uk`
- Homepage has trailing slash: `https://bathplumbers.co.uk/`
...
```bash
curl https://bathplumbers.co.uk/robots.txt
```

**Expected content:**
- `User-agent: *` → `Allow: /`
- `Disallow: /admin`, `/login`, `/reset-password`, `/status`
- `Sitemap: https://bathplumbers.co.uk/sitemap.xml`
...
- [x] Exactly 1 `<h1>` containing "Blocked Drains Bath"
...
- [x] Service area section (Somerset and Wiltshire + districts)
...
| `src/components/sections/BathFAQSection.tsx` | Expanded from 6 to 12 FAQs |
