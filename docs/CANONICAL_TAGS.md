...
This document confirms the canonical tag strategy for bathplumbers.co.uk.
...
|-----------|------------------|---------|
| Homepage | `https://bathplumbers.co.uk/` | With trailing slash |
| All other pages | `https://bathplumbers.co.uk/path` | No trailing slash |

### Examples

```
Homepage:        https://bathplumbers.co.uk/
Services:        https://bathplumbers.co.uk/services
Service detail:  https://bathplumbers.co.uk/services/blocked-drains
Location:        https://bathplumbers.co.uk/locations/bristol
Location+Service: https://bathplumbers.co.uk/locations/bristol/blocked-drains
```
...
| `/locations/bristol` | `/locations/bristol` |
...
// Should return: https://bathplumbers.co.uk/current-path
```

### 2. View Source
Check the `<head>` section for exactly ONE canonical tag:
```html
<link rel="canonical" href="https://bathplumbers.co.uk/services/blocked-drains">
```
...
for url in "/" "/services" "/services/blocked-drains" "/locations" "/locations/bristol"; do
  echo "Checking: $url"
  curl -s "https://bathplumbers.co.uk$url" | grep -o '<link rel="canonical"[^>]*>'
done
```
...
- `/locations/bristol` → canonical: `/locations/bristol`
- `/locations/radstock` → canonical: `/locations/radstock`
...
- `/locations/bristol/blocked-drains` → canonical: `/locations/bristol/blocked-drains`
- `/locations/radstock/blocked-drains` → canonical: `/locations/radstock/blocked-drains`
...
- ✅ HTTPS only
- ✅ Consistent domain (bathplumbers.co.uk, no www)
